...
def summarize(rows: list[dict]) -> list[dict]:
    """Build one summary row per provider combination.

    If any row has a non-empty prompt_mode column, the grouping key becomes
    (stt_provider, tts_provider, prompt_mode) so compact vs compact_v2 vs full
    can be compared side by side. Otherwise it stays (stt_provider, tts_provider).
    """
    has_prompt_mode = any((row.get("prompt_mode") or "").strip() for row in rows)

    groups = defaultdict(list)
    for row in rows:
        combo = f"{row.get('stt_provider', '')}+{row.get('tts_provider', '')}"
        if has_prompt_mode:
            combo = f"{combo}+{row.get('prompt_mode', '') or '-'}"
