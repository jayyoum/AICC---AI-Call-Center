...
def main():
    ...
    run_id = new_run_id()
    result = route_with_ollama(transcript, routes=routes, model=args.model, prompt_mode=args.prompt_mode)
    ...
    print(f"Prompt mode: {result.get('prompt_mode')}   Route tree used: {result['route_tree_used']}   Route valid: {result.get('route_valid')}")
